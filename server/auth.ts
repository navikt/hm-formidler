import { Client, generators, Issuer, TokenSet, TokenSetParameters } from 'openid-client'
...
let idportenClient: Nullable<Client> = null
...
async function setup(idpConfig: IDPortenConfig, txConfig: TokenXConfig, appConf: AppConfig): Promise<void> {
  if (process.env.NAIS_CLUSTER_NAME === 'labs-gcp') return
    ...
    const { idporten, tokenx } = await init()
    idportenClient = idporten
    tokenxClient = tokenx
  }
}
...
async function init() {
  ...
  try {
    ...
    const idportenJwk = idportenConfig.clientJwk && JSON.parse(idportenConfig.clientJwk)
...
async function refresh(oldTokenSet: TokenSetParameters): Promise<TokenSet> {
  if (idportenConfig == null || idportenClient == null || idportenMetadata == null || tokenxConfig == null) {
    throw new Error('setup() må kalles først')
  }

  try {
    const tokenSet = await idportenClient.refresh(new TokenSet(oldTokenSet), {
...
function authUrl(session: SessionData): string {
  if (idportenConfig == null || idportenClient == null) {
    throw new Error('setup() må kalles først')
  }
...
async function validateOidcCallback(req: Request): Promise<TokenSet> {
  if (idportenConfig == null || idportenClient == null || idportenMetadata == null) {
    throw new Error('setup() må kalles først')
  }
  ...
  try {
    return await idportenClient.callback(idportenConfig.redirectUri, params, { nonce, state }, additionalClaims)
  } catch (err: unknown) {
...
const setIsAuthenticated: RequestHandler = async (req, res) => {
  ...
  req.isAuthenticated = !!tokenSet && !tokenSet.expired()
...
const logoutHandler =
  (destroySessionBySid: (sid: string) => void): RequestHandler =>
  async (req, res) => {
    if (typeof req.query.sid === 'string') await destroySessionBySid(req.query.sid)
    res.sendStatus(200)
  }
...
const callbackHandler = (): RequestHandler => async (req, res) => {
  const session = req.session
  try {
    const tokens = await auth.validateOidcCallback(req)
